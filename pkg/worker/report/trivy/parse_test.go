					...
					Vulnerabilities: []v1alpha2.Vulnerability{
						{
							...
							VulnerabilityCommon: v1alpha1.VulnerabilityCommon{
								...
								Description:      "The function PEM_read_bio_ex() reads a PEM file from a BIO and parses and decodes the \"name\" (e.g. \"CERTIFICATE\"), any header data and the payload data. If the function succeeds then the \"name_out\", \"header\" and \"data\" arguments are populated with pointers to buffers containing the relevant decoded ...",
						...
						},
						{
							...
							VulnerabilityCommon: v1alpha1.VulnerabilityCommon{
								...
								Description:      "Certifi is a curated collection of Root Certificates for validating the trustworthiness of SSL certificates while verifying the identity of TLS hosts. Certifi prior to version 2023.07.22 recognizes \"e-Tugra\" root certificates. e-Tugra's root certificates were subject to an investigation prompted by ...",
					...
					Vulnerabilities: []v1alpha2.Vulnerability{
						...
						},
						{
							...
							VulnerabilityCommon: v1alpha1.VulnerabilityCommon{
								...
								Description:      "Affected versions of `npm-registry-fetch` are vulnerable to an information exposure vulnerability through log files. The cli supports URLs like `\u003cprotocol\u003e://[\u003cuser\u003e[:\u003cpassword\u003e]@]\u003chostname\u003e[:\u003cport\u003e][:][/]\u003cpath\u003e`. The password value is not redacted and is printed to stdout and also to any generated ...",
...
func TestTruncate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "ok",
			args: args{s: "123456789", max: 100},
			want: "123456789",
		},
		{
			name: "truncate",
			args: args{s: "123456789", max: 5},
			want: "12345...",
		},
		{
			name: "max negative",
			args: args{s: "123456789", max: -1},
			want: "",
		},
		{
			name: "max zero",
			args: args{s: "123456789", max: 0},
			want: "",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := truncate(tt.args.s, "...", tt.args.max); got != tt.want {
